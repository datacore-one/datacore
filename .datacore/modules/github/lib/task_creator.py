...
def _make_task_id(repo: str, number: int) -> str:
    """Stable identity for a triaged GitHub item: gh-{repo_short}-{number}.

    The date used to be part of this id, which made it not an identity at all:
    the same issue produced a different id every day, so it never matched its
    own existing task. On 2026-08-10 — the first successful run after a 23-day
    outage — that appended a second copy of the body to an already-DONE entry
    from 2026-07-08 while reporting "created: 1".

    An id answers WHAT this is about, not WHEN it was filed. The date is kept
    as TRIAGE_DATE for reporting. _find_task_by_id still matches the legacy
    "<id>-YYYY-MM-DD" form so tasks written under the old scheme dedupe too.
    """
    repo_short = repo.split("/")[-1] if "/" in repo else repo
    return f"gh-{repo_short}-{number}"
