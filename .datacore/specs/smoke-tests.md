...
| # | Workflow | Command | Pass condition |
|---|----------|---------|----------------|
| S10 | Fleet sync | `python3 .datacore/lib/git_fleet_sync.py ~/Data --execute --pull` (every host) | every repo `clean (pulled)`; no PULL CONFLICT. SKIPs are repos on feature branches — a decision, not a fault. |
...
| S28 | Every host can push what it writes | per host+space: `git push --dry-run origin main` | succeeds. Open gap: winston commits to `1-datafund` and `3-fds` but cannot push (HTTPS remotes, nothing stored, `gh` not logged in, SSH key rejected). |
