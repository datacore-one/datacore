...
| S32 | No forked ledger logs | `python3 .datacore/lib/git_relay.py --forks` (EVERY machine — needs no SSH or remote) | `0 ledger file(s) with forked (actor,seq)`. Two events sharing one `(actor,seq)` is a fork, never a merge (DIP-0046). Union-merging a per-writer log creates them: 138 on 2026-08-30. Run this on every host — fork checking used to live only in winston's twice-daily v2-verify, and the first fleet-wide run found 9 on nightshift that nothing else would have caught. |
...
| S30 | Systemd units healthy | `systemctl --failed` (NS, WN) | **empty**. All three 2026-08-30 failures were real and are fixed: `coinglass-scraper` (venv `PYTHONPATH` hid the `datacore` package), `datacore-fleet-sync` (was failing permanently on staleness — now fails only when work is actually at risk), `plur-geo-scan` (two OpenRouter passes legitimately exceed 2h; cap raised to 4h — if a run ever reaches 4h, that IS a hang). |
...
| Date | Run by | Result |
|------|--------|--------|
| 2026-08-30 (robustness) | Claude (audit session) | Closed the class the audit exposed: **work trapped behind access gaps**. `git_relay.py` added — the operator machine relays commits from hosts that cannot reach their own remotes. **166 commits were trapped** (135 winston/5-plur, 20 winston/1-datafund, 8 plur-claw/2-plur-space, 2 winston/3-fds, 1 plur-claw/8-firm, plus 1 in 1-datafund/2-projects/website); all relayed and verified on origin. Fleet-sync now separates NO ACCESS / UNRELATED HISTORY / PULL CONFLICT and fails only when work is actually at risk. **Final: 0 trapped, 0 failed units on either host, lint 0, checkpoints 9/9, no stale locks.** |
