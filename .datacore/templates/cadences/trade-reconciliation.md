...
Match every trade recorded internally since the last reconciliation against the exchange's own fills, and name each mismatch.
...
4. **Write the report**, headed `# Trade Reconciliation — YYYY-MM-DD`, with:
   - `## Market snapshot`
   - `## Trades in the period`: internal vs exchange
   - `## Mismatches and their cause`
   - `## Position log after reconciliation`
