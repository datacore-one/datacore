...
this cadence -- it goes to cos approval and today's post is an evergreen one instead.
...
2. **Choose:** the best-ranked ready draft in `1-tracks/geo/drafts/` that is not yet in
   ...
   (submit those drafts for approval with `cos_approval_submit` instead).
...
7. **Record:** write `1-tracks/comms/blog/blog-post-YYYY-MM-DD.md`, headed
   `# Blog post — YYYY-MM-DD`, with the title, both URLs, and what the fact-check removed.
