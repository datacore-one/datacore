...
def test_undeclared_output_is_retained_without_publication_authority(repo: Path):
    ...
    assert (repo / 'mystery.txt').read_text() == 'x\n'
    assert 'mystery.txt' in git(repo, 'status', '--porcelain')
