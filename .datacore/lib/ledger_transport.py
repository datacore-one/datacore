...
def _converge_locked(space: Path) -> Result:
    ...
    if rc != 0:
        ...
        return Result(False, "fetch failed (offline?)", {"stderr": err.strip()[:200]})
    ...
    if autosaved:
        _git(space, "add", "-A")
        _git(space, "commit", "-m", "ledger: autosave before converge")
...
if __name__ == "__main__":
    ...
    ap.add_argument("op", choices=["converge", "gaps", "classify"])
