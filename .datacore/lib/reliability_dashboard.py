...
_PAGE = """<title>Datacore Fleet Record</title>
...
<footer>
  ...
  reliability scoreboard, the principals registry, the job manifest, cadence liveness,
  fleet status, the per-writer event logs and the task queue. Read-only: it computes
  nothing of its own and writes no state.
</footer>
</div>
"""
