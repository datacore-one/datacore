    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                print(f"\nToken may be expired or revoked. To re-authenticate:")
                print(f"  1. Delete the token file: {TOKEN_FILE}")
                print(f"  2. Re-run this script: python {__file__} setup")
                print(f"  3. Complete the OAuth flow in your browser")
                sys.exit(1)
        else:
