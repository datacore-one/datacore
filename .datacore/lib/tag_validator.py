...
class TagValidator:
    ...
    # Uppercase prefixes that are known org-mode conventions (not unknown tags).
    # Only these uppercase prefixes are allowed; others are flagged.
    KNOWN_UPPERCASE_PREFIXES = {'AI', 'CRM', 'URGENT', 'HOME', 'WORK'}

    def __init__(self, data_dir: str = None):
        self.data_dir = Path(data_dir or os.environ.get("DATACORE_ROOT", os.path.expanduser("~/Data")))
        self.config_dir = self.data_dir / ".datacore"
        self.registry_path = self.config_dir / "tags.yaml"

        self.registry = {}
        self.known_org_tags: Set[str] = set()
        self.known_org_tag_segments: Set[str] = set()  # individual segments (e.g. 'research', 'AI')
    ...
    def _load_registry(self):
        ...
        for section_name, section in self.registry.items():
            ...
            for tag_name, tag_def in section.items():
                ...
                # Collect org tags and their individual segments
                if 'org' in tag_def:
                    org_tag = tag_def['org']
                    self.known_org_tags.add(org_tag)
                    # Extract individual segments (e.g. ':AI:research:' -> {'AI', 'research'})
                    for seg in org_tag.strip(':').split(':'):
                        if seg:
                            self.known_org_tag_segments.add(seg)
    ...
    def _all_segments_known(self, segments: List[str]) -> bool:
        """Check if all segments of a compound tag are individually known.

        A segment is known if it appears as a segment in any registered org tag,
        starts with a known uppercase prefix (e.g. AI, CRM), or is a person
        context tag (@name).
        This handles org-mode multi-tags like :readwise:strategy: where each
        segment is a separate tag that may be individually registered.
        """
        # Build case-insensitive lookup once (cached per call is fine, set is small)
        lower_segments = {s.lower() for s in self.known_org_tag_segments}
        for seg in segments:
            # Check if segment is directly known
            if seg in self.known_org_tag_segments:
                continue
            # Case-insensitive match (e.g. 'ai' matches 'AI' segment)
            if seg.lower() in lower_segments:
                continue
            # Check known uppercase prefixes
            if seg.upper() in self.KNOWN_UPPERCASE_PREFIXES:
                continue
            # Person context tags (@name) are always valid
            if seg.startswith('@'):
                continue
            # Unknown segment
            return False
    ...
    # Regex for lines inside property drawers / logbooks.
    # These are :KEY: value lines that are NOT heading tags.
    _PROPERTY_LINE_RE = re.compile(r'^\s*:[A-Za-z_][A-Za-z0-9_]*:\s')
    _DRAWER_BOUNDARY_RE = re.compile(r'^\s*:(PROPERTIES|END|LOGBOOK):\s*$')
    ...
    def validate_org_files(self) -> Dict[str, List[Tuple[str, int, str]]]:
        ...
        # Discover spaces dynamically from filesystem
        org_dirs = sorted(
            p / "org" for p in self._discover_space_dirs()
            if (p / "org").is_dir()
        )
            ...
            for org_file in org_dir.glob("*.org"):
                with open(org_file, 'r', errors='ignore') as f:
                    ...
                    for line_num, line in enumerate(f, 1):
                        stripped = line.strip()
                        # Track property drawer / logbook context
                        if self._DRAWER_BOUNDARY_RE.match(line):
                            if ':END:' in stripped:
                                in_drawer = False
                            else:
                                in_drawer = True
                            continue
                        # Skip all lines inside property drawers / logbooks
                        if in_drawer:
                            continue
                        # Only validate heading lines (tags live on headings)
                        # and top-level tag-only lines (filetags)
                        ...
                        for tag in tags:
                            ...
                            elif tag not in self.known_org_tags:
                                # Check if it's a valid tag
                                if not re.match(r'^:[A-Za-z@]', tag):
                                    continue
                                # Compound tags: split ':seg1:seg2:' into segments
                                # and validate each individually
                                segments = [s for s in tag.strip(':').split(':') if s]
                                if self._all_segments_known(segments):
                                    continue
    ...
    def validate_notes(self) -> Dict[str, List[Tuple[str, int, str]]]:
        ...
        # Discover note directories dynamically from all spaces
        notes_dirs = sorted(
            notes_dir
            for p in self._discover_space_dirs()
            for notes_dir in [p / "notes", p / "knowledge"]
            if notes_dir.is_dir()
        )
    ...
    def get_tag_stats(self) -> Dict[str, int]:
        ...
        space_dirs = self._discover_space_dirs()
        ...
        # Count hashtags
        for space in space_dirs:
            for notes_dir in [space / "notes", space / "knowledge"]:
                if not notes_dir.exists():
                    continue
                for md_file in notes_dir.rglob("*.md"):
                    try:
    ...
    def generate_report(self) -> str:
        ...
        if all_unknown:
            ...
            report.append("\n  Consider adding these to .datacore/tags.yaml")
        else:
