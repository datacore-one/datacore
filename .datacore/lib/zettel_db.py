...
# Space configurations - dynamically discovered from filesystem
def _discover_spaces(data_root=None):
    """Discover spaces from [0-9]-* directories under data_root (default: DATA_ROOT)."""
    ...
    for space_dir in entries:
        if not space_dir.is_dir() or not space_dir.name[:1].isdigit():
            continue
        # Use directory name without number prefix as key
        name = space_dir.name.split('-', 1)[1] if '-' in space_dir.name else space_dir.name
        # Build scan paths from known subdirectories
        scan_paths = []
        for sub in ['3-knowledge/zettel', '3-knowledge/pages', '3-knowledge/clippings',
                     '3-knowledge/reference', '3-knowledge/literature',
                     '0-inbox', '1-active', '1-tracks/research',
                     'journal', 'notes/journals']:
            p = space_dir / sub
            if p.is_dir():
                scan_paths.append(p)
...
def detect_author(content, frontmatter, path):
    ...
    if frontmatter:
        ...
        if author in ['ai', 'claude', 'gpt', 'chatgpt'] or 'claude' in author or 'gpt' in author:
            return 'ai'
        elif author in ['human', OWNER_NAME] or OWNER_NAME in author:
            return 'human'
