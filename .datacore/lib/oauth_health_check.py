...
def check_claude_token(now, warn_days):
    ...
    try:
        ...
        return ((name, 'UNREADABLE', str(e)), f"{name}: unreadable ({e})", 1)
    if not exp_ms:
...
def main():
    ...
    # --- Claude Code (Anthropic Max) OAuth token that powers Miles ---
    crow, cproblem, cexit = check_claude_token(now, args.warn_days)
    if crow:
        rows.append(crow)
    if cproblem:
        problems.append(cproblem)
    exit_code = max(exit_code, cexit)
