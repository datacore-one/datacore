...
1. **Preferred**: Call `plur_admin` MCP tool with `action` = `"plur_inject_hybrid"`, `prompt` = your task description, `scope` = `agent:archiver`
2. **Fallback**: If MCP is unavailable, read `.datacore/state/agent-engrams/archiver.md` for compiled engrams

Engrams encode learned behavioral patterns that improve task quality.

## Agent Context

### When to Reference Archiving Guidelines

**Always reference when:**
- Moving content to `3-archive/`
- Adding archive frontmatter
- Creating bidirectional version links
- Updating archive index

**Key decisions this spec informs:**
- Mirror folder structure requirement
- Version history format
- Archive frontmatter fields
- No nested archives rule

### Quick Reference

| Question | Answer |
|----------|--------|
| Where is archive? | `[space]/3-archive/` |
| Where is archive spec? | `.datacore/specs/archiving-guidelines.md` |
| Archive index? | `3-archive/README.md` |
| Key frontmatter? | `archived`, `archived_from`, `archived_reason` |

### Related DIPs

- [DIP-0002](../dips/DIP-0002-layered-context-pattern.md) - Space structure
- [DIP-0009](../dips/DIP-0009-gtd-specification.md) - Journal logging

### Related Agents

| Agent | Relationship |
|-------|--------------|
| `context-maintainer` | May trigger after major archiving |
| `scaffolding-auditor` | May identify content for archiving |

### Integration Points

- **Archiving spec** - Follows `.datacore/specs/archiving-guidelines.md`
- **Journal** - Logs archiving actions
- **Version history** - Maintains bidirectional links
