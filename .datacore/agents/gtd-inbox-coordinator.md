...
1. **Preferred**: Call `plur_inject_hybrid` MCP tool with `prompt` = your task description and `scope` = `agent:gtd-inbox-coordinator`
...
<Task>
  ...
  prompt: |
    ...
    ** Check out this article on data marketplaces
       ...
       Saw this on HN, looks relevant to Organization
    ```
