...
1. **Preferred**: Call `datacore.inject` MCP tool with `prompt` = your task description and `scope` = `agent:learning-absorber`
2. **Fallback**: If MCP is unavailable, read `.datacore/state/agent-engrams/learning-absorber.md` for compiled engrams
